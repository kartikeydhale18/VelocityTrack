...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        // Fetch or create user role in Firestore using a real-time listener
        const userRef = doc(db, 'users', firebaseUser.uid);
        
        unsubSnapshot = onSnapshot(userRef, async (userSnap) => {
          if (userSnap.exists()) {
            ...
            // The snapshot listener will automatically catch this and update state!
          }
        });
      } else {
